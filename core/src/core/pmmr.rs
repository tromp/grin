...
/// A Mountain proof.
/// Proves inclusion of a leaf node in a mountain under a peak node in an MMR.
...
pub struct MountainProof {
	...
	pub node: Hash,
	/// The sibling hashes along the path from node to peak
	pub path: Vec<Hash>,
}
...
impl Writeable for MountainProof {
...
impl Readable for MountainProof {
	fn read(reader: &mut Reader) -> Result<MountainProof, ser::Error> {
